...
void RigidBodyDriverUtilityTrait<Scalar>::computeJacobianMatrix(RigidBodyDriver<Scalar, 3>* driver, CompressedJacobianMatrix<Scalar, 3>& J, DimensionTrait<3> trait)
{
    ...
    for(unsigned int i = 0; i < m; ++i)
    {
        ...
        angular_normal_lhs = (contact_point->globalContactPosition() - rigid_body->globalMassCenter()).cross(contact_point->globalContactNormalLhs());
		rigid_body = driver->rigidBody(object_rhs);
        angular_normal_rhs = (contact_point->globalContactPosition() - rigid_body->globalMassCenter()).cross(contact_point->globalContactNormalRhs());
...
void RigidBodyDriverUtilityTrait<Scalar>::computeFricJacobianMatrix(RigidBodyDriver<Scalar, 3>* driver, CompressedJacobianMatrix<Scalar, 3>& D, DimensionTrait<3> trait)
{
    ...
    for(unsigned int i = 0; i < m; ++i)
    {
        ...
        for(unsigned int k = 0; k< fric_sample_count; ++k)
        {
            ...
            rigid_body = driver->rigidBody(object_lhs);
            angular_normal_lhs = (contact_point->globalContactPosition() - rigid_body->globalMassCenter()).cross(sample_normal);  // 摩擦雅克比矩阵  左物体
			rigid_body = driver->rigidBody(object_rhs);
            angular_normal_rhs = (contact_point->globalContactPosition() - rigid_body->globalMassCenter()).cross(-sample_normal); // this is where the friction-reverse-bug was fixed，修改了原先系统中摩擦力反向的bug
			D.setFirstValue(i * fric_sample_count + k, object_lhs, sample_normal, angular_normal_lhs);                            // 摩擦雅克比矩阵  右物体 
...
void RigidBodyDriverUtilityTrait<Scalar>::computeCoefficient(RigidBodyDriver<Scalar, 3>* driver, VectorND<Scalar>& CoR, VectorND<Scalar>& CoF, DimensionTrait<3> trait)
{
    ...
    for(unsigned int i = 0; i < m; ++i)
    {
        ...
        CoR[i] = min(cor_lhs, cor_rhs); // mud drop on steel floor, it does not bounce up
        for(unsigned int j = 0; j< fric_sample_count; ++j)
        {
            CoF[i * fric_sample_count + j] = max(cof_lhs, cof_rhs); // I'm suspicious: whether there will be friction between a glued box and ice surface
        }
    }
}
